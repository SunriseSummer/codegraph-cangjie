  ...
  if (!useDefaults) {
    ...
    if (shouldInstallGlobally) {
      ...
      try {
        ...
        execSync('npm install -g @cangjie-lang/codegraph', { stdio: 'pipe', windowsHide: true, timeout: 120_000 });
        s.stop('Installed codegraph CLI on PATH');
      } catch {
        s.stop('Could not install (permission denied)');
        clack.log.warn('Try: sudo npm install -g @cangjie-lang/codegraph');
      }
    } else {
    ...
    if (plan.summary.length > 0) {
      ...
      if (removeBinaries) {
        ...
        if (result.npm === 'removed') {
          ...
          clack.log.warn('npm uninstall failed — run `npm uninstall -g @cangjie-lang/codegraph` yourself (EACCES usually means it needs sudo).');
        }
        ...
        clack.log.info('Kept the CLI. Remove it later with `codegraph uninstall` or `npm uninstall -g @cangjie-lang/codegraph`.');
      }
    }
  }
