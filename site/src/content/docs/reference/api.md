...
CommonJS works too — `const { CodeGraph } = require('@cangjie-lang/codegraph');`.
...
import {
  ...
  FileLock,
} from '@cangjie-lang/codegraph';
```

## Embedding requirements

- **Install from npm** (`npm i @cangjie-lang/codegraph`) so the matching per-platform package — which carries the compiled library — is fetched alongside the shim.
