 ...
 * `require("@cangjie-lang/codegraph")` on their OWN Node by re-exporting the
 * compiled library that ships inside the per-platform optionalDependency
 * (@cangjie-lang/codegraph-<target>/lib/dist/index.js).
 *
...
// SDK's `require.resolve('@cangjie-lang/codegraph-<target>/...')` walks to it.
function makeConsumer(): { root: string; mainPkg: string } {
  const root = mkTmp('consumer');
  const mainPkg = path.join(root, 'node_modules', '@cangjie-lang', 'codegraph');
  fs.mkdirSync(mainPkg, { recursive: true });
  fs.copyFileSync(SDK_SRC, path.join(mainPkg, 'npm-sdk.js'));
  fs.writeFileSync(
    path.join(mainPkg, 'package.json'),
    JSON.stringify({ name: '@cangjie-lang/codegraph', version: VERSION, main: 'npm-sdk.js' }) + '\n'
  );
  return { root, mainPkg };
}
...
function installPlatformPackage(root: string, sentinel: string): void {
  const pkgRoot = path.join(root, 'node_modules', '@cangjie-lang', `codegraph-${target}`);
  writeFakeLib(path.join(pkgRoot, 'lib', 'dist'), sentinel);
  fs.writeFileSync(
    path.join(pkgRoot, 'package.json'),
    JSON.stringify({ name: `@cangjie-lang/codegraph-${target}`, version: VERSION }) + '\n'
  );
}
...
describe('npm-sdk programmatic entry', () => {
  ...
  it('throws an actionable error when no bundle is installed or cached', () => {
    ...
    expect(r.stderr).toContain(`@cangjie-lang/codegraph-${target}`);
