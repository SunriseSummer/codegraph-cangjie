...
describe('Installer targets — partial-state idempotency', () => {
  ...
  it('claude: uninstall strips stale hooks written in the npx form (local)', () => {
    ...
    const file = seedSettings('local', {
      hooks: {
        PostToolUse: [
          { matcher: 'Edit|Write', hooks: [{ type: 'command', command: 'npx @cangjie-lang/codegraph mark-dirty', async: true }] },
        ],
        Stop: [
          { hooks: [{ type: 'command', command: 'npx @cangjie-lang/codegraph sync-if-dirty' }] },
        ],
      },
    });
